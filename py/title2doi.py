...
def main():
    ...
    parser.add_argument("-m", "--match_threshold", type=float,
                        default=0.9, help=ARG_HELP_STRINGS["match_threshold"])
    parser.add_argument("-a", "--ask_threshold", type=float,
                        default=0.8, help=ARG_HELP_STRINGS["ask_threshold"])
    parser.add_argument("-c", "--colors", type=bool,
                        default=COLORS_DEFAULT, help=ARG_HELP_STRINGS["ansi_colors"])
    parser.add_argument("--start", type=int, default=0,
                        help=ARG_HELP_STRINGS["start"])
    parser.add_argument("--end", type=int, default=inf,
    ...
    with open(args.title_file, "r") as f:
        ...
        for field in reader.fieldnames:
            if field.lower() in TITLE_HEADER_WL:
                print(colorise("Using column '" + field +
                      "' as title column", "green"))
                title_field = field
                break
        else:
            print(colorise(
                "ERROR: Could not find a column name which might denote a title column", "red"))
            sys.exit()
        ...
        for line in reader:
            ...
            if result["similarity"] == 1.0:
                ...
                msg_head = msg_head.format(
                    round(result["similarity"], 2)).ljust(L_JUST)
        ...
        if ask_count > 0:
            ...
            ask_msg = ask_msg.format(
                ask_count, args.ask_threshold, args.match_threshold)
        ...
        for line in modified_lines:
            if line["ask"]:
                ...
                for i in range(len(diff)):
                    ...
                    xref_print = colorise_text_segment(
                        xref_print, b, b + c, color)
                query_head = colorise(
                    "line {}, query title:".format(line["line_num"]), "blue")
                xref_head = colorise("Possible match ({}):".format(
                    round(line["similarity"], 2)), "yellow")
        ...
        with open("out.csv", "w") as out:
            ...
            writer = csv.DictWriter(
                out, header, extrasaction='ignore', dialect=dialect)
            writer.writeheader()
...
def crossref_query_title(title):
    ...
    request = Request(url)
    request.add_header(
        "User-Agent", "OpenAPC DOI Importer (https://github.com/OpenAPC/openapc-de/blob/master/python/import_dois.py; mailto:openapc@uni-bielefeld.de)")
    try:
        ret = urlopen(request)
        content = ret.read()
...
def colorise_text_segment(text, start, end, color):
    ...
    if color not in ANSI_COLORS.keys():
        raise ValueError(
            "Argument 'color' must be one of [" + ", ".join(ANSI_COLORS.keys()) + "]")
    s = ANSI_COLORS[color]
    e = "\033[0m"
